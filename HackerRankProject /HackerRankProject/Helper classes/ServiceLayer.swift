...
//This is the service/network class(lets call it utility) 

//enum to handle all service response scenarios
enum RequestableResult<T> {
    ...
    //computed property to return the value associated with each success cases
    public var value: T? {
        switch self {
...
extension ServiceLayer {
    ...
    func serviceRequestWithURL(url: String, completion: @escaping (RequestableResult<Any?>) -> Void) {
        ...
        //create shared singleton session object as we dont need to set the cache/cookie policies here
